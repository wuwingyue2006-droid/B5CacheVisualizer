...
- [x] 可以稳定地前进和回看已生成的帧；
- [x] 自动播放、暂停、继续、停止和速度切换都有唯一且明确的状态；
- [x] 回看帧与当时的地址、路径、Cache 表和统计一致；
- [x] Reset、Apply Config、Trace 变更后不会继续播放旧时间线；
- [x] 对话框只负责事件连接与刷新，时间线状态集中在展示控制器中；
- [x] 没有修改核心公共接口。

## 实现与自测记录

- 新增 `VisualizationFrame`，逐帧复制保存请求、访问结果、统计、两级配置与 Cache 行状态；
- 新增 `VisualizationController`，集中管理历史定位、Stopped / Playing / Paused / Reviewing 状态与三档速度；
- 使用唯一 MFC Timer 驱动自动播放，切换速度时先安全停止旧 Timer；
- 配置或 Trace 改变时立即终止播放并清空旧时间线；
- Debug x64 编译结果：0 警告、0 错误；
- 核心回归测试：61/61 通过；
- 实际界面已验证单步、连续前进、历史回看、从历史位置自动续播、结束自动停止和 Reset 清空。
