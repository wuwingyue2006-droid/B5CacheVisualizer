...
代号和固定分支名确认后，成员可以随时从最新 `dev` 创建并上传该分支，不需要等待组长单独批准。

## 六、第一次接入仓库

第一次接入的目标是让本地电脑拥有最新 `dev`，并立即创建和上传本人固定的 `feature-*` 分支。
...
此时已经具备创建个人分支的条件，直接继续下一节。不要在 `dev` 上修改或提交代码。
...
5. 核对 origin；
6. 切换并更新 dev；
7. 根据我的 A–E 代号确认固定分支名；
8. 从最新 dev 创建并上传该 feature 分支；
9. 确认当前分支和工作区状态。

严格限制：
- 不修改代码；
- 除第一次上传本人固定 feature 分支外，不 commit 或 push 其他内容；
- 不执行 git init；
- 不覆盖已有目录；
- 不索要、显示或保存密码和访问令牌；
- 不使用任何强制或清理命令。
