...
> 适用对象：B5 五人小组全体成员
> 当前阶段目标：所有成员完成仓库接入，并从最新 `dev` 创建自己的固定 `feature-*` 分支。

## 一、先看最重要的结论

五人分工和分支名已经固定。全组统一按下面的顺序推进：
...
切换到 dev 并拉取最新内容
    ↓
从最新 dev 创建各自的 feature-* 分支
    ↓
第一次 push 后向组长报告
```

成员确认自己的 A–E 代号后，可以随时创建对应的固定 `feature-*` 分支，不需要等待组长再次批准。不能自行发明其他分支名，也不能创建多个个人分支。
...
## 十一、完成接入后随时创建个人 feature 分支

假设某位成员代号是 B，对应固定分支为 `feature-mapping`，完成接入后直接执行：
...
- 不执行 git init；
- 不 commit；
- 除第一次上传本人固定 feature 分支外，不 push 其他内容；
...
现在先问我：成员代号、是否已接受邀请、准备使用的本人姓名和邮箱、计划把项目放在哪个父目录。根据代号使用固定分支名，然后从 git --version 开始，一步一步指导我。
```
